...
> 本文记录2026-07-20之前的30类规则基线审核，文中的22/30、25/30和21,408条标签均为历史实绩，不应改写。后续39规则扩展扫描、34类正式技能与5类候选规则定稿，以及33,914条最终正式标签结果见`seed-detection-expanded-review.md`。
...
- WSL项目`.venv`已恢复默认`dev`测试组和正式AV2依赖，`av2==0.3.6`可导入；
- 299项自动测试通过；
- Python编译检查通过；
- 真实AV2单场景端到端烟测通过；
